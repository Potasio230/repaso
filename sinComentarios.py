...
def inventarioMarca(marca):
    ...
    for codigo, datos in vehiculos.items():
        marcaVehiculos = datos[0].lower()
        if marcaVehiculos == marcaBuscada:
...
def busquedaPrecio(precioMin, precioMax):
    ...
    if len(vehiculosEncontrados) > 0:
        print(f"Los vehículos en el rango de precios consultados son: {vehiculosEncontrados}")
    else:
    ...
    while True:
        ...
        if opcion == 1:
            ...
            while continuar:
                ...
                precioNuevo = obtenerNumeroEntero("Ingrese el nuevo precio: ")
